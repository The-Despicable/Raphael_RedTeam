#!/usr/bin/env python3
"""
D9 Regression Test: LLM Entity-ID Wiring for Causal Integration

Re-runs the 15 FULL_RAPHAEL episodes from the D7-R1 Gemma re-test
plus 15 NO_LLM episodes for baseline comparison.

Success criteria (per D9_LLM_ENTITY_WIRING_REPAIR_SPEC.json):
  1. semantic_inference_driven rationale code appears in ≥1 PlanDecision
     per FULL_RAPHAEL episode where the LLM is invoked.
  ...
  4. NO_LLM mean score must remain statistically unchanged (0.500 ± 0.05).
  5. FULL mean score: any value (Rule 3 — causal connection, not benchmark).
"""
...
def run_episode(template_key, seed, architecture, llm_config):
    """Run a single episode with D9 entity_ids wiring active."""
...
def main():
    results_file = "/home/yaser/raphael-2.0/arena/results/d9_regression_results.json"
    ...
    print("=" * 72)
    print("D9 REGRESSION TEST: LLM Entity-ID Wiring")
    print("=" * 72)
    ...
    print(f"  Episodes with semantic_inference_driven: {len(full_si_driven)}/{len(full_results)}")
    print(f"  Total semantic_inference_driven appearances: {full_si_total}")
    print()
    for r in full_results:
        ...
        print(f"  {prohib_status}{si_status} {r['template']}|{r['seed']}: "
    ...
    for template_key in TEMPLATES:
        for seed in EPISODES[template_key]:
            ...
            if f and n:
                ...
                print(f"  {template_key}|{seed}: FULL={f['score']:.2f}{fp}[{si}] "
    ...
    # Criterion 1: semantic_inference_driven in ≥1 PlanDecision per FULL episode
    c1_pass = len(full_si_driven) == len(full_results)
    c1_detail = (f"({len(full_si_driven)}/{len(full_results)} episodes have it, "
                 f"{full_si_total} total appearances)")
    print(f"[C1] semantic_inference_driven in ≥1 PlanDecision per FULL episode: "
    ...
    # Criterion 2: D7 invariant (zero persistent denials = no repetitions)
    c2_pass = (full_persistent == 0)
    print(f"[C2] D7 invariant (zero persistent denials = no repetitions): "
          f"{'✅ PASS' if c2_pass else '⚠️ NOTE'} "
          f"({full_persistent} persistent denials across all FULL episodes)")
    
    # Criterion 3: D8 invariant (prohibited_attempted == 0)
    c3_pass = len(full_prohibited) == 0
    print(f"[C3] D8 invariant (prohibited_attempted = 0/15): "
          f"{'✅ PASS' if c3_pass else '❌ FAIL'} "
          f"({len(full_prohibited)}/15 episodes still have it)")
    
    # Criterion 4: NO_LLM statistically unchanged (0.500 ± 0.05)
    c4_pass = 0.450 <= no_llm_mean <= 0.550
    print(f"[C4] NO_LLM mean ({no_llm_mean:.3f}) within 0.500 ± 0.05: "
          f"{'✅ PASS' if c4_pass else '⚠️ INVESTIGATE'}")
    ...
    # Note: C1 is the critical D9 criterion. C2 and C3 must also pass.
    # C4 is advisory (NO_LLM stability check). FULL score check removed per Rule 3.
    overall = c1_pass and c2_pass and c3_pass and llm_pass and len(failures) == 0
    
    if overall:
        print("=" * 72)
        print("OVERALL: ✅ D9 REGRESSION PASS")
        print("=" * 72)
        ...
        print()
        print("D9 repair verified. LLM entity_ids wiring is live.")
        print("The semantic_inference_driven rationale code now reaches the Planner.")
    else:
        print("=" * 72)
        print("OVERALL: ❌ D9 REGRESSION FAIL")
        print("=" * 72)
        if not c1_pass:
            print(f"  C1 FAIL: {len(full_si_driven)}/{len(full_results)} episodes have SI-driven rationale")
        if not c2_pass:
            print(f"  C2 FAIL: {full_persistent} persistent denials (D7 invariant broken)")
        if not c3_pass:
            print(f"  C3 FAIL: {len(full_prohibited)}/15 episodes still have prohibited_attempted")
        if not llm_pass:
    ...
    # Save results
    output = {
        "spec": "D9_LLM_ENTITY_WIRING_REPAIR_SPEC",
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "overall_pass": overall,
        "criteria": {
            "c1_semantic_inference_driven": c1_pass,
            "c2_d7_invariant": c2_pass,
            "c3_d8_invariant": c3_pass,
            "c4_no_llm_stable": c4_pass,
